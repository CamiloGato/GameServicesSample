...
{
    [ExecuteAlways]
    public class BlocksModule : CozyModule, ICozyBiomeModule
    {
        ...
        public List<BlocksModule> biomes = new List<BlocksModule>();
        ...
        public override void InitializeModule()
        {
            ...
            biomes = FindObjectsByType<BlocksModule>(FindObjectsSortMode.None).Where(x => x != this).ToList();
        }
        ...
        {
            weatherSphere.GetModule<BlocksModule>().biomes = FindObjectsByType<BlocksModule>(FindObjectsSortMode.None)
                .Where(x => x != weatherSphere.GetModule<BlocksModule>()).ToList();
        }

        public void RemoveBiome()
        {
            weatherSphere.GetModule<BlocksModule>().biomes.Remove(this);
        }
        ...
        {
            if (!weatherSphere.GetModule<BlocksModule>())
            {
    ...
    public class E_BlocksModule : E_CozyModule, E_BiomeModule
    {
        ...
        public static bool colorBlocksSettingsWindowOpen;
        public static bool developmentToolkit;
        #endregion
        BlocksModule t;
        ...
        public override void DisplayInCozyWindow()
        {
            ...
            selection = EditorGUILayout.BeginFoldoutHeaderGroup(selection, "    Selection Settings", EditorUtilities.FoldoutStyle);
            ...
            {
                colorBlocksSettingsWindowOpen = EditorGUILayout.BeginFoldoutHeaderGroup(colorBlocksSettingsWindowOpen, "    Color Blocks", EditorUtilities.FoldoutStyle);
                EditorGUILayout.EndFoldoutHeaderGroup();

                if (colorBlocksSettingsWindowOpen)
                {
                ...
                developmentToolkit = EditorGUILayout.BeginFoldoutHeaderGroup(developmentToolkit, "    Development Toolkit", EditorUtilities.FoldoutStyle);
                EditorGUILayout.EndFoldoutHeaderGroup();

                if (developmentToolkit)
                {
        ...
        public void DrawInlineBiomeUI()
        {
            ...
            EditorGUI.indentLevel++;
            EditorGUILayout.PropertyField(serializedObject.FindProperty("useSingleBlock"));
            if (t.useSingleBlock)
                EditorGUILayout.PropertyField(serializedObject.FindProperty("testColorBlock"));
            else
            {
